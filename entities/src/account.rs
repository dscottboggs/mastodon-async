...
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq, PartialOrd, Ord, is_enum_variant)]
...
mod tests {
    ...
    fn test_account_example() {
        ...
        let subject: Account = serde_json::from_str(example).unwrap();
        assert_eq!(subject.id, AccountId::new("23634"));
        assert_eq!(subject.username, "noiob");
        assert_eq!(subject.acct, "noiob@awoo.space");
        assert_eq!(subject.display_name, "ikea shark fan account");
